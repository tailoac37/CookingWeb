...
public class CategoriesAPI {
	...
	public String addCategories(@RequestHeader("Authorization") String auth , @RequestPart("image") MultipartFile image , @RequestPart("categories") CategoriesRequest categories) throws IOException
	{
		String token = auth.replace("Bearer", "") ; 
		return service.addCategories(token, image, categories) ; 
	}
	@PatchMapping("/api/admin/categories/{id}")
	public String updateCategories(@RequestHeader("Authorization") String auth , @RequestPart(value ="image" , required = false ) MultipartFile image , @RequestPart(value = "categories" , required = false) CategoriesRequest categories , @PathVariable("id") Integer Id ) throws IOException
	{
		String token = auth.replace("Bearer", "") ; 
		return service.updateCategories(token, image, categories, Id)  ; 
	}
	@DeleteMapping("/api/admin/categories/{id}")
	public String delCategories(@RequestHeader("Authorization") String auth , @PathVariable("id") Integer Id ) 
	{
		String token = auth.replace("Bearer", "") ; 
		return service.delCategories(token, Id) ; 
	}
