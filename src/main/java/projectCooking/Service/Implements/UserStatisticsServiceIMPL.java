...
public class UserStatisticsServiceIMPL implements UserStatisticsService {
    ...
    public UserStatisticsResponse getOverviewStatistics(Integer Id) {
        ...
        response.setApprovedRecipes(recipesRepo.getCountRecipeByUserAndStatus(user.getUserId(), Recipe.RecipeStatus.APPROVED));
        response.setPendingRecipes(recipesRepo.getCountRecipeByUserAndStatus(user.getUserId(), Recipe.RecipeStatus.PENDING));
        response.setRejectedRecipes(recipesRepo.getCountRecipeByUserAndStatus(user.getUserId(), Recipe.RecipeStatus.REJECTED));
    ...
    public RecipeStatisticsResponse getRecipeStatistics(Integer Id) {
        ...
        Map<String, Long> difficultyBreakdown = recipes.stream()
            ...
            .collect(Collectors.groupingBy(r -> r.getDifficultyLevel().name(), Collectors.counting()));
            ...
            .collect(Collectors.groupingBy(
                ...
                Collectors.collectingAndThen(
                    ...
                    list -> {
                        ...
                        perf.setAverageViews(list.stream().mapToInt(Recipe::getViewCount).average().orElse(0));
                        perf.setAverageLikes(list.stream().mapToInt(Recipe::getLikeCount).average().orElse(0));
                        return perf;
                    }
                )
            ));
