...
public class CategoriesServcieImplements implements CategoriesService {
	...
	public String delCategories(String token, Integer Id) {
		...
		if(categories.getImageUrl() != null)
		{
			cloudinaryService.deleteImageByUrl(categories.getImageUrl())  ; 
		}
		categoriesRepo.delete(categories);
		AdminAction adminAction = new  AdminAction()  ; 
		adminAction.setActionType(ActionType.CATEGORIES);
		adminAction.setAdmin(user);
		adminAction.setAdminNote("Vừa xóa 1 categories") ; 
		adminActionRepo.save(adminAction)  ; 
		notifService.sendChangeNotificationToAdmins(user, "Vừa mới xóa 1 categories  !!");
		return "done";
	}
